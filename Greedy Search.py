...
# Definir función heurística para la búsqueda codiciosa (distancia de Manhattan)
def heuristic(a, b):
...
# Definir algoritmo de búsqueda codiciosa
#En este algoritmo, utilizamos una lista (frontier) para almacenar los nodos que aún no han sido explorados,
#con sus prioridades (calculadas utilizando la heurística) como tuplas.
#En cada iteración, seleccionamos el nodo con la menor prioridad y lo expandimos.
#Si encontramos el nodo objetivo, detenemos la búsqueda. Una vez que se completa la búsqueda,
#reconstruimos el camino desde el punto de inicio hasta el punto objetivo.
def greedy(start, goal, mars_map, scale):
    start = xy_to_rc(*start, scale)
    goal = xy_to_rc(*goal, scale)

    frontier = [(heuristic(start, goal), start)]  # Utilizamos solo la heurística como prioridad
    came_from = {}
    visited = set()
    ...
    while frontier:
        ...
        for next_node in get_neighbors(*current_node, mars_map, scale):
            if next_node not in visited:
                priority = heuristic(next_node, goal)  # Utilizamos solo la heurística como prioridad
...
# Calcular ruta utilizando búsqueda codiciosa
path, distance = greedy(start_point, end_point, mars_map, 10.0174)
